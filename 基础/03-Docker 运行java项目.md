...
# 2、java镜像环境运行方式
docker run -d -p 3001:3001 --restart=always -v /yolo/code/demo/app.jar:/tmp/app.jar --name springboot java:latest java -jar /tmp/app.jar

# 3、可远程调试运行方式
docker run -d -p 5000:5000 -p 50001:50001 --name demo \
-v /yolo/code/demo/app.jar:/tmp/app.jar \
java:latest \
